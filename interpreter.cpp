...
int main(){
    ...
    bool isFin = false;
     for (size_t i = 0; true; i++) {
        try 
        {
            ...
            //if((contents[i] >=96 && contents[i] <= 125) ||(contents[i] >= 65 && contents[i] <= 90) || contents[i] == 95 || contents[i] == 33)
            //if(contents[i] >= 33 && contents[i] <= 127)
                //ofs << contents[i];
            //else
              //  ofs << "wasd" << endl;
    ...
    /*ofs.open("modifiedResults.txt");

    int wtf =  F.size() - MI;

    for(int i = 150; i < wtf; i++){
